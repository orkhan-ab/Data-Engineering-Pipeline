...
def create_catalog_description(csv_dist: str, ttl_dist: str) -> Graph:
    result = Graph()
    ...
    create_distributions(result, csv_dist, ttl_dist)
...
def create_distributions(collector: Graph, csv_dist: str, ttl_dist: str) -> None:
    # CSV
    ...
    with open(csv_dist, "rb") as file:
        checksum = sha1(file.read()).hexdigest()
        checksum_node = BNode()
        collector.add((d1, SPDX.checksum, checksum_node))
        collector.add((checksum_node, RDF.type, SPDX.Checksum))
        collector.add((checksum_node, SPDX.algorithm, SPDX.checksumAlgorithm_sha1))
        collector.add((checksum_node, SPDX.checksumValue, Literal(checksum, datatype=XSD.hexBinary)))
    ...
    with open(ttl_dist, "rb") as file:
        checksum = sha1(file.read()).hexdigest()
        checksum_node = BNode()
        collector.add((d2, SPDX.checksum, checksum_node))
        collector.add((checksum_node, RDF.type, SPDX.Checksum))
        collector.add((checksum_node, SPDX.algorithm, SPDX.checksumAlgorithm_sha1))
        collector.add((checksum_node, SPDX.checksumValue, Literal(checksum, datatype=XSD.hexBinary)))
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        logging.error("Usage: python script.py <csv_distribution> <ttl_distribution> <output_file_path>")
        sys.exit(1)

    catalog = create_catalog_description(csv_dist=sys.argv[1], ttl_dist=sys.argv[2])
    catalog.serialize(format="turtle", destination=sys.argv[3])
